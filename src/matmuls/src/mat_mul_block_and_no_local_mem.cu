...
int32_t MatMulBlockAndNoLocalMem(Mat &mat0, Mat &mat1, Mat &dst)
{
    ...
    LOG_INFO("*** block size: ", block_size.x, block_size.y, " grid size: ", grid_size.x, grid_size.y);

    // top left
    MatMulTopLeft<<<grid_size, block_size>>>(mat0, mat1, dst);
    ...
    if (offset_x < w)
    {
        ...
        MatMulTopRight<<<grid_tr_size, block_tr_size>>>(mat0, mat1, dst);
    }
    ...
    if (offset_y < h)
    {
        ...
        MatMulDownLeft<<<grid_dl_size, block_dl_size>>>(mat0, mat1, dst);
    }
    ...
    if (offset_x < w && offset_y < h)
    {
        ...
        MatMulDownRight<<<grid_dr_size, block_dr_size>>>(mat0, mat1, dst);
    }
    ...
    if (err != cudaSuccess)
    {
        LOG_INFO("CUDA kernel error: ", cudaGetErrorString(err));
        return ret;
    }
