...
CUDA_GLOBAL void ReduceIdleThreadFreeKernel(Mat *mat, Mat *sum)
{
...
int32_t ReduceIdleThreadFree(Mat &mat, int32_t &sum)
{
    ...
    ReduceIdleThreadFreeKernel<BLOCK_SIZE><<<global_size, block_size>>>(mat.GetMatAllOnCUDAMem(), sum_mat_gpu.GetMatAllOnCUDAMem());
